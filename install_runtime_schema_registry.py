...
TARGET = PACKAGE_DIR / "registry.py"
...
REQUIRED_FILES = [
    ...
    PACKAGE_DIR / "ownership.py",
    PACKAGE_DIR / "change_detection.py",
    PACKAGE_DIR / "compatibility.py",
    PACKAGE_DIR / "deprecation.py",
    PACKAGE_DIR / "audit.py",
    PACKAGE_DIR / "snapshots.py",
    PACKAGE_DIR / "ports.py",
]

EXPECTED_BASELINE_SIZE = 41251

EXPECTED_BASELINE_SHA256 = (
    "f882d1f050f74448b7f5c8af9019ec9d"
    "33de4e8468b955e87a4cdbf2a8d3edf5"
)
...
BACKUP = (
    ...
    / "data"
    / "runtime_backups"
    / f"runtime_schema_registry_install_{TIMESTAMP}"
    / TARGET.name
)
...
def verify_behavior(
    ...
    version_one = make_definition(
        definitions_module,
        namespace="product.schemas",
        name="record",
        version="1.0.0",
        owner_id="schema_service",
        state="active",
        fields={
            "identifier": {
                "type": "string",
    ...
    assert (
        snapshot.schema_namespace_count
        == 1
    )
    ...
    runtime_registry.register_namespace(
        "runtime.schema",
        "runtime_schema",
        "runtime_schema",
        runtime_actor=True,
    ...
    runtime_definition = make_definition(
        definitions_module,
    ...
    runtime_registry.register_schema(
        runtime_definition,
        "runtime_schema",
        runtime_actor=True,
    )
    ...
    concurrent_registry.register_namespace(
        "product.concurrent",
        "concurrent_service",
        "concurrent_service",
    )
    ...
    def register_subject(
        ...
        try:
            ...
            concurrent_registry.register_schema(
                definition,
                "concurrent_service",
            )
...
def main() -> int:
    ...
    print("REGISTRY.PY INSTALLATION AND REVIEW")
    print("=" * 78)
    ...
    if not TARGET.exists():
        raise FileNotFoundError(
            f"registry.py does not exist: {TARGET}"
        )
    ...
    try:
        ...
        print(
            "The registry.py installation failed, "
            "so Claude's original registry was restored."
        )
        print()
        print(
    ...
    print("Nova registry replacement:      PASS")
    print("registry.py compilation:        PASS")
    print("Package import:                 PASS")
    print("Port contract implementation:   PASS")
    print("Namespace authorization:        PASS")
    print("Reserved runtime namespaces:    PASS")
    print("Schema ownership enforcement:   PASS")
    print("Version uniqueness:             PASS")
    print("Version ordering:               PASS")
    print("Compatibility enforcement:      PASS")
    print("Required bump enforcement:      PASS")
    print("Transactional rollback:         PASS")
    print("Append-only audit integration:  PASS")
    print("Lifecycle transition policy:    PASS")
    print("Deprecation policy binding:     PASS")
    print("Inactive lookup enforcement:    PASS")
    print("Canonical version listings:     PASS")
    print("Deterministic fingerprinting:   PASS")
    print("Snapshot measurements:          PASS")
    print("Registry integrity verification: PASS")
    print("Concurrent registration safety: PASS")
    print()
    print(f"Backup file: {BACKUP}")
    print()
    print(
        "REGISTRY.PY: INSTALLED, "
