...
routes_dir = Path("backend/server/routes")
routes_dir.mkdir(parents=True, exist_ok=True)

p = routes_dir / "site_workspace.py"
...
def connect_domain(payload: ConnectDomainRequest) -> Dict[str, Any]:
    ...
    job_result = execute_universal_knowledge_job_v1(job)
...
main = Path("backend/server/main.py")
main_code = main.read_text(encoding="utf-8-sig").replace("\ufeff", "")

import_line = "from .routes.site_workspace import router as site_workspace_router\n"
include_line = "app.include_router(site_workspace_router)\n"

if import_line not in main_code:
    main_code += "\n" + import_line

if include_line not in main_code:
    main_code += include_line

main.write_text(main_code, encoding="utf-8")

print("Added /api/site/workspace/connect_domain route and included router in main.py")
