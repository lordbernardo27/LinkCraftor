...
EVIDENCE_DIR = (
    ...
    / "runtime"
    / "uri_phase_1"
    / "1_1_11_runtime_capability_negotiation"
)
...
BACKUP_DIR = (
    BACKUP_ROOT
    / f"uri_phase1_1_11_runtime_capability_negotiation_{TIMESTAMP}"
)
...
EVIDENCE_JSON = (
    EVIDENCE_DIR
    / f"runtime_capability_negotiation_build_{TIMESTAMP}.json"
)

EVIDENCE_TEXT = (
    EVIDENCE_DIR
    / f"runtime_capability_negotiation_build_{TIMESTAMP}.txt"
)
...
def verify_behavior(module) -> list[tuple[str, str]]:
    results: list[tuple[str, str]] = []

    Version = module.RuntimeCapabilityVersion
    VersionRange = module.RuntimeCapabilityVersionRange
    Capability = module.RuntimeCapability
    Requirement = module.RuntimeCapabilityRequirement
    Level = module.RuntimeCapabilityRequirementLevel
    Status = module.RuntimeCapabilityStatus
    Manifest = module.RuntimeCapabilityManifest
    Registry = module.RuntimeCapabilityRegistry

    version = Version.parse("1.2.3")

    assert str(version) == "1.2.3"
    assert Version.parse("1.2.4") > version
    ...
    accepted_range = VersionRange(
        minimum=Version.parse("1.0.0"),
        maximum=Version.parse("2.0.0"),
    )

    assert accepted_range.contains(
        Version.parse("1.5.0")
    )

    assert not accepted_range.contains(
        Version.parse("3.0.0")
    )

    prerelease_range = VersionRange(
        minimum=Version.parse("1.0.0"),
        maximum=Version.parse("2.0.0"),
        allow_prerelease=False,
    )

    assert not prerelease_range.contains(
    ...
    consumer = Manifest(
        component_id="runtime-controller-1",
        component_type="runtime-controller",
        component_version=Version.parse("1.0.0"),
        runtime_version=Version.parse("1.0.0"),
        capabilities=(),
    )

    provider = Manifest(
        component_id="worker-1",
        component_type="runtime-worker",
        component_version=Version.parse("1.5.0"),
        runtime_version=Version.parse("1.0.0"),
        capabilities=(
            Capability(
                name="job.execute",
    ...
    requirements = (
        Requirement(
            ...
            accepted_protocols=frozenset(
                {"runtime-rpc-v2"}
            ),
            accepted_serialization_formats=frozenset(
                {"json"}
            ),
    ...
    report = module.negotiate_runtime_capabilities(
        consumer_manifest=consumer,
        provider_manifest=provider,
        requirements=requirements,
    )
    ...
    assert any(
        match.status is Status.SATISFIED
        for match in report.matches
    )

    assert any(
        match.status is Status.DEPRECATED
        for match in report.matches
    )

    assert any(
        match.status is Status.MISSING
        for match in report.matches
    )
    ...
    missing_required_report = (
        module.negotiate_runtime_capabilities(
            consumer_manifest=consumer,
            provider_manifest=provider,
            requirements=(
                Requirement(
    ...
    assert missing_required_report.compatible is False

    try:
        missing_required_report.require_compatible()
    except module.RuntimeCapabilityNegotiationError:
        pass
    else:
        raise AssertionError(
            "Missing required capability was not rejected."
        )
    ...
    assert version_mismatch_report.compatible is False

    assert any(
        match.status is Status.VERSION_MISMATCH
        for match in version_mismatch_report.matches
    )
    ...
    assert protocol_report.compatible is False

    assert any(
        match.status is Status.PROTOCOL_MISMATCH
        for match in protocol_report.matches
    )
    ...
    assert serialization_report.compatible is False

    assert any(
        match.status
        is Status.SERIALIZATION_MISMATCH
        for match in serialization_report.matches
    )

    results.append(
        (
            "Serialization compatibility enforcement",
            "PASS",
        )
    )
    ...
    assert schema_report.compatible is False

    assert any(
        match.status is Status.SCHEMA_MISMATCH
        for match in schema_report.matches
    )
    ...
    assert report.fingerprint == report_two.fingerprint
    assert provider.fingerprint == provider.fingerprint

    results.append(
        (
            "Deterministic manifests and reports",
            "PASS",
        )
    )

    try:
        provider.component_id = "modified"
    except Exception:
        pass
    else:
        raise AssertionError(
            "Capability manifests must be immutable."
        )
    ...
    try:
        ...
        raise AssertionError(
            "Negotiation reports must be immutable."
        )
    ...
    registry_report = registry.negotiate(
        consumer_component_id=(
            consumer.component_id
        ),
        provider_component_id=(
    ...
    assert registry_report.compatible is True
    ...
    results.append(
        (
            "Capability registry and audit history",
            "PASS",
        )
    )
    ...
    def negotiate_in_thread() -> None:
        try:
            for _ in range(100):
                threaded_report = registry.negotiate(
                    consumer_component_id=(
                        consumer.component_id
                    ),
                    provider_component_id=(
                    ...
                    requirements=requirements,
                    actor="thread-verifier",
                )

                if not threaded_report.compatible:
                    raise AssertionError(
                        "Threaded negotiation became incompatible."
                    )
        except Exception as exc:
    ...
    results.append(
        (
            "Thread-safe capability negotiation",
            "PASS",
        )
    )

    replacement_provider = Manifest(
        component_id=provider.component_id,
        component_type=provider.component_type,
        component_version=Version.parse("1.6.0"),
        runtime_version=Version.parse("1.0.0"),
    ...
    registry.remove(
        replacement_provider.component_id,
        actor="builder-verifier",
    )
    ...
    except module.RuntimeCapabilityNotFoundError:
        pass
    else:
        raise AssertionError(
            "Removed manifest remained registered."
        )

    results.append(
        (
            "Controlled manifest replacement and removal",
            "PASS",
        )
    )
...
def write_evidence(
    *,
    ...
    lines = [
        ...
        "UNIVERSAL RUNTIME INFRASTRUCTURE",
        (
            "1.1.11 — RUNTIME CAPABILITY "
            "NEGOTIATION BUILD EVIDENCE"
        ),
        "=" * 78,
        "",
    ...
    lines.extend(
        [
            ...
            "INTEGRATION STATUS",
            "-" * 78,
            "Service Registry integration:      PENDING",
            "Worker Registry integration:       PENDING",
...
def main() -> int:
    ...
    print(
        "1.1.11 — RUNTIME CAPABILITY NEGOTIATION BUILD"
    )
    print("=" * 78)
    ...
    try:
        ...
        verification_results.append(
            (
                "Capability Negotiation compilation",
                "PASS",
            )
        )
        ...
        print(
            "The 1.1.11 build failed, so the previous "
            "Capability Negotiation file was restored."
        )
        print()
        print(error_text)
    ...
    print("FILES")
    print("-" * 78)
    print(f"Capability negotiation: {TARGET}")
    print(f"Evidence JSON:          {EVIDENCE_JSON}")
    print(f"Evidence text:          {EVIDENCE_TEXT}")
    print()
    print("1.1.11 RUNTIME CAPABILITY NEGOTIATION")
    print("IMPLEMENTATION: PASS")
    print("ISOLATED VERIFICATION: PASS")
    print("CAPABILITY MANIFESTS: PASS")
    print("REQUIRED CAPABILITY ENFORCEMENT: PASS")
    print("OPTIONAL CAPABILITY WARNINGS: PASS")
    print("VERSION NEGOTIATION: PASS")
    print("PROTOCOL NEGOTIATION: PASS")
    print("SERIALIZATION NEGOTIATION: PASS")
    print("SCHEMA NEGOTIATION: PASS")
    print("DEPRECATED CAPABILITY WARNINGS: PASS")
    print("DETERMINISTIC REPORTS: PASS")
    print("THREAD SAFETY: PASS")
    print("SERVICE REGISTRY INTEGRATION: PENDING")
    print("WORKER REGISTRY INTEGRATION: PENDING")
