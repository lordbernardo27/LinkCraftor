...
def main(
    ...
    single_paragraph_result = (
        synthetic_validation(
            article_text=(
                " ".join(
                    f"single_paragraph_word_{index}"
    ...
    single_paragraph_valid = (
        single_paragraph_result.get(
            "status"
        )
        == "PASS"
        and single_paragraph_result.get(
            "checks",
            {},
        ).get(
    ...
    if not single_paragraph_valid:
        failures.append(
            "A valid single-paragraph article was rejected."
        )

    repeated_paragraph = " ".join(
        f"repeated_content_word_{index}"
        for index in range(30)
    )

    repeated_content_result = (
        synthetic_validation(
            article_text=(
                repeated_paragraph
                + "\n\n"
            ...
            title="Intentional Repeated Content",
        )
    )

    duplicate_ratio_non_blocking = (
        repeated_content_result.get(
            "status"
        )
        == "PASS"
        and "duplicate_paragraph_ratio"
        not in repeated_content_result.get(
            "checks",
            {},
        )
        and "HIGH_DUPLICATE_PARAGRAPH_RATIO"
        not in repeated_content_result.get(
            "rejection_reasons",
            [],
        )
        and repeated_content_result.get(
            "observations",
            {},
        ).get(
    ...
    if not duplicate_ratio_non_blocking:
        failures.append(
            "Duplicate paragraph ratio still affected PASS or FAIL."
        )
    ...
    print(
        "Single-paragraph article valid:    "
        + (
            "PASS"
            if report[
                "single_paragraph_valid"
