"""Website Article Integrity Phase 4.4 package."""

from .website_article_structure_validator import (
    PHASE as STRUCTURE_PHASE,
    PHASE_NAME as STRUCTURE_PHASE_NAME,
    VALIDATOR_VERSION as STRUCTURE_VALIDATOR_VERSION,
    StructureValidationResult,
    run_structure_validation,
    validate_article_structure,
)

from .website_article_component_validator import (
    PHASE as COMPONENT_PHASE,
    PHASE_NAME as COMPONENT_PHASE_NAME,
    VALIDATOR_VERSION as COMPONENT_VALIDATOR_VERSION,
    ComponentValidationResult,
    run_component_validation,
    validate_required_components,
)

from .website_article_corruption_truncation_detector import (
    PHASE as CORRUPTION_TRUNCATION_PHASE,
    PHASE_NAME as CORRUPTION_TRUNCATION_PHASE_NAME,
    VALIDATOR_VERSION as CORRUPTION_TRUNCATION_VALIDATOR_VERSION,
    CorruptionTruncationResult,
    inspect_article,
    run_corruption_truncation_detection,
)

from .website_integrity_report_generator import (
    PHASE as REPORT_PHASE,
    PHASE_NAME as REPORT_PHASE_NAME,
    REPORT_VERSION,
    generate_website_integrity_report,
)

from .website_article_quarantine_manager import (
    PHASE as QUARANTINE_PHASE,
    PHASE_NAME as QUARANTINE_PHASE_NAME,
    QUARANTINE_VERSION,
    execute_quarantine,
)

__all__ = [
    "STRUCTURE_PHASE",
    "STRUCTURE_PHASE_NAME",
    "STRUCTURE_VALIDATOR_VERSION",
    "StructureValidationResult",
    "run_structure_validation",
    "validate_article_structure",
    "COMPONENT_PHASE",
    "COMPONENT_PHASE_NAME",
    "COMPONENT_VALIDATOR_VERSION",
    "ComponentValidationResult",
    "run_component_validation",
    "validate_required_components",
    "CORRUPTION_TRUNCATION_PHASE",
    "CORRUPTION_TRUNCATION_PHASE_NAME",
    "CORRUPTION_TRUNCATION_VALIDATOR_VERSION",
    "CorruptionTruncationResult",
    "inspect_article",
    "run_corruption_truncation_detection",
    "REPORT_PHASE",
    "REPORT_PHASE_NAME",
    "REPORT_VERSION",
    "generate_website_integrity_report",
    "QUARANTINE_PHASE",
    "QUARANTINE_PHASE_NAME",
    "QUARANTINE_VERSION",
    "execute_quarantine",
]
