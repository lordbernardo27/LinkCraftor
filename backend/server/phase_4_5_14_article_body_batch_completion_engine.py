...
WEBSITE_UCD_DIR = ROOT / "backend" / "server" / "data" / "website_unified_content_store"
...
def _extract_validated_body(record: Dict[str, Any]) -> Optional[str]:
    ...
    candidates = [
        ...
        record.get("primary_content"),
        record.get("cleaned_article_body"),
        record.get("validated_article_body"),
    ]
...
def _is_validated(record: Dict[str, Any]) -> bool:
    ...
    # If the Website Unified Content Store already contains article_body/primary_content,
    # accept it as pipeline-produced content, but do not perform extraction here.
...
def complete_article_body_store_from_validated_website_content_v1() -> Dict[str, Any]:
    ...
    index: Dict[str, Any] = _read_json(ARTICLE_BODY_INDEX) or {
        "version": "1.0",
        "store": "universal_article_body_store",
        "items": {},
    }

    ledger: Dict[str, Any] = _read_json(ARTICLE_BODY_LEDGER) or {
        "version": "1.0",
        "events": [],
    }
        ...
        try:
            ...
            if not _is_validated(record):
                ...
                ledger["events"].append({
                    ...
                    "reason": "not_validated_by_website_pipeline",
                    "timestamp": _utc_now(),
                })
                continue
            ...
            if not body:
                ...
                ledger["events"].append({
                    ...
                    "document_id": doc_id,
                    "source_url": source_url,
                    "reason": "missing_validated_article_body",
                    "timestamp": _utc_now(),
                })
                continue
            ...
            index["items"][body_id] = {
                "document_id": doc_id,
                "source_url": source_url,
                "body_path": str(body_path.as_posix()),
                "character_count": len(body),
                "word_count": len(body.split()),
                "source": "website_unified_content_store",
                "hydration_method": "validated_pipeline_copy_only",
                "updated_at": _utc_now(),
            }
    ...
    return {
        ...
        "skipped": skipped,
        "failed": failed,
        "article_body_index": str(ARTICLE_BODY_INDEX),
        "article_body_ledger": str(ARTICLE_BODY_LEDGER),
        "architectural_rule": "No HTML parsing, extraction, cleaning, or validation occurs in this engine.",
    }
