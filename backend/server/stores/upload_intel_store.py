...
BOUNDARY_WEAK_WORDS: Set[str] = {
    ...
    "it", "its", "their", "there", "here", "when", "where", "why", "how", "what", "which", "who"
}
...
def _is_numeric_fragment(tokens: List[str]) -> bool:
    ...
    joined = " ".join(tokens)
    if re.fullmatch(r"(?:\d+|day|days|week|weeks|month|months|year|years)(?:\s+(?:\d+|day|days|week|weeks|month|months|year|years))*", joined):
        return True
...
def _looks_like_broken_continuation(tokens: List[str]) -> bool:
    if not tokens:
        return True

    weak_starts = {"and", "or", "but", "about", "this", "that", "these", "those", "find", "after", "before"}
    weak_ends = {"and", "or", "but", "this", "that", "these", "those", "the", "a", "an", "to", "of", "for", "from"}
...
def _looks_like_sentence_candidate_source(sentence: str) -> bool:
    s = _canonical_phrase(sentence)
    if not s:
        return False
    if _looks_like_ui_junk(s):
        return False

    toks = _tokenize(s)
    if len(toks) < NGRAM_MIN_N:
        return False
    if _numeric_ratio(toks) >= 0.6:
        return False
    return True


def _is_good_phrase_shape(tokens: List[str]) -> bool:
    if not tokens:
        return False
    ...
    if _bad_boundary(tokens):
        return False
    if _looks_like_broken_continuation(tokens):
        return False
...
def _qualify_phrase(raw_phrase: str) -> Tuple[bool, str]:
    ...
    if not _is_good_phrase_shape(tokens):
        return False, ""
...
def _generate_sentence_candidates(sentence: str) -> List[str]:
    if not _looks_like_sentence_candidate_source(sentence):
        return []
...
def _dominance_filter(phrases: List[str]) -> List[str]:
    uniq = sorted(set(_canonical_phrase(p) for p in phrases if _canonical_phrase(p)), key=lambda x: (-len(x.split()), x))
    kept: List[str] = []
    ...
    for p in uniq:
        ...
        for k in kept:
            ...
            # If p is fully contained in a longer kept phrase and is weak/partial, drop it.
            if len(k_toks) >= len(p_toks) + 2 and " ".join(p_toks) in " ".join(k_toks):
                # protect a few short medically meaningful phrases
                if len(p_toks) <= 2 and p in {
...
def _upsert_phrase_record(
    ...
    rec["quality_score"] = _quality_score_for(str(rec.get("source_type") or source_type), int(rec["count_total"]))
...
# ============================================================
# PUBLIC ENTRYPOINT: called from files.py at upload time
# ============================================================
...
def build_upload_intelligence(
    ...
    # --------
    # STRUCTURED PHRASE HARVESTING (Tier A)
    # --------
    ...
    for raw_text, source_type, section_id, snippet in structured_candidates:
        ok, phrase = _qualify_phrase(raw_text)
        if ok and phrase:
            _upsert_phrase_record(ph, phrase, source_type, doc_id, section_id, snippet)
            upsert_node(f"phrase:{phrase}", "PHRASE", phrase)

    # --------
    # PARAGRAPH + SENTENCE SCAN
    # --------
    for i, para in enumerate(paragraphs):
        pid = f"p{i}"
        para_snippet = para

        # Sentence-level qualified generation
        sentence_candidates_all: List[str] = []
        for sidx, sentence in enumerate(_split_sentences(para)):
            sentence_candidates = _generate_sentence_candidates(sentence)
            sentence_candidates_all.extend(sentence_candidates)

            # give extra support to sentence-leading topic fragments
            lead = _canonical_phrase(sentence[:120])
            ok, lead_phrase = _qualify_phrase(lead)
            if ok and lead_phrase:
                sentence_candidates_all.append(lead_phrase)
    ...
    filtered_phrases = set(_dominance_filter(all_phrases))

    # Keep all Tier A even if dominance would normally drop them
    tier_a = {k for k, v in ph.items() if isinstance(v, dict) and str(v.get("tier") or "") == "A"}
    filtered_phrases |= tier_a

    ph_final: Dict[str, Any] = {}
    for k, v in ph.items():
        if k in filtered_phrases:
            ph_final[k] = v
