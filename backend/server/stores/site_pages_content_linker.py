...
def link_site_pages_to_unified_content_v1(*, workspace_id: str, site_pages_path: str | None=None) -> Dict[str, Any]:
    ...
    for page in pages:
        ...
        page['unified_content_ref'] = {'content_id': page['content_id'], 'source_type': 'website_crawl'}
        linked += 1
    data['pages'] = pages
    data['content_linking'] = {'linked_count': linked, 'skipped_count': skipped, 'linked_at_utc': datetime.now(timezone.utc).isoformat()}
    data['updated_at'] = datetime.now(timezone.utc).isoformat()
    path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'status': 'passed', 'workspace_id': workspace_id, 'site_pages_path': str(path), 'linked_count': linked, 'skipped_count': skipped}
