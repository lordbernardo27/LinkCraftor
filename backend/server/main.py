...
from backend.server.routes.external.import_clear import router as import_clear_runtime_router
...
OWNER_COOKIE = "lc_owner_auth"


def _get_owner_key() -> str:
    return (
        os.environ.get("LINKCRAFTOR_OWNER_KEY")
...
if not _already_mounted("backend.server.routes.external.sitemap_import", "/api/external/sitemap_import_runtime"):
    app.include_router(
...
if not _already_mounted("backend.server.routes.external.import_clear", "/api/external/import_clear_runtime"):
    app.include_router(
        import_clear_runtime_router,
        prefix="/api/external/import_clear_runtime",
    )


from .routes.site_workspace import router as site_workspace_router
app.include_router(site_workspace_router)
