...
def run_universal_knowledge_queue_v1(
    *,
    ...
    """
    Drain up to ``max_jobs`` queued jobs for a workspace.

    Selection rules:
      - only jobs with status == "queued" are eligible
      - if ``job_type`` is given, only jobs of that type are eligible
      - eligible jobs are ordered by ascending priority when
        ``order_by_priority`` is true (stable, so FIFO within a priority)
      - everything not executed is written back to the queue unchanged
    """
    ws = safe_id(workspace_id)
    runner_id = f"runner_{uuid.uuid4().hex[:16]}"

    queued = read_queue(ws, limit=10000)
    pending, remaining = _partition_queue(
        queued,
        job_type=job_type,
    ...
    for job in pending:
        result = _execute_one(job, runner_id)
    ...
    return {
        ...
        "order_by_priority": bool(order_by_priority),
