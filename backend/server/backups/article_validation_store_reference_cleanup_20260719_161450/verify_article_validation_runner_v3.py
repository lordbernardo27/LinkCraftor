...
def active_store_import_references(
) -> list[str]:
    references: list[str] = []
...
def main() -> int:
    ...
    incorrect_store_removed = (
        not INCORRECT_STORE_PATH.exists()
    )

    active_store_references = (
        active_store_import_references()
    )
    ...
    if not incorrect_store_removed:
        failures.append(
            "Incorrect Article Validation Store v3 "
            "still exists."
        )

    if active_store_references:
        failures.append(
            "Active source files still reference "
            "article_validation_store_v3."
        )
    ...
    print(
        "Incorrect Store v3 removed:         "
        + (
            "PASS"
            if incorrect_store_removed
            else "FAIL"
        )
    )

    print(
        "Active Store v3 imports removed:    "
        + (
            "PASS"
            if not active_store_references
            else "FAIL"
        )
    )
