...
async def upload_file(
    ...
    # Preserve a top-level public error/detail message when one exists,
    # without exposing nested internal pipeline structures.
    if internal_result.get("error"):
        public_response["error"] = str(
            internal_result.get("error")
        )[:500]

    if internal_result.get("detail"):
        public_response["detail"] = str(
